 ...
 * provider
 */
public class PaTicketThroughProvider {
	 ...
	 * @return
	 */
	public String insertPaTicketThrough(PaTicketThrough entity) {
		SQL sql = new SQL().INSERT_INTO("pa_ticket_through");
	 ...
	 * @return
	 */
	public String selectPaTicketThroughByCount(PaTicketThrough entity) {
		SQL sql = new SQL().SELECT("count(*)").FROM("pa_ticket_through");
	 ...
	 * @return
	 */
	public String selectPaTicketThroughByLimt(PaTicketThrough entity) {
		SQL sql = new SQL().SELECT("*").FROM("pa_ticket_through");
					if(!Objects.isNull(entity.getId())) {sql.WHERE("id = #{id}");}
			if(!Strings.isNullOrEmpty(entity.getName())) {sql.WHERE("name = #{name}");}
			if(!Objects.isNull(entity.getPrice())) {sql.WHERE("price = #{price}");}
			if(!Strings.isNullOrEmpty(entity.getType())) {sql.WHERE("type = #{type}");}
	 ...
	 * @return
	 */
	public String selectPaTicketThrough(PaTicketThrough entity) {
		SQL sql = new SQL().SELECT("*").FROM("pa_ticket_through");
			if(!Objects.isNull(entity.getId())) {sql.WHERE("id = #{id}");}
			if(!Strings.isNullOrEmpty(entity.getName())) {sql.WHERE("name = #{name}");}
			if(!Objects.isNull(entity.getPrice())) {sql.WHERE("price = #{price}");}
	 ...
	 * @return
	 */
	public String updatePaTicketThrough(PaTicketThrough entity) {
		SQL sql = new SQL().UPDATE("pa_ticket_through");
				sql.SET("name = #{name}");
		sql.SET("price = #{price}");
		sql.SET("type = #{type}");
	 ...
	 * @return
	 */
	public String updatePaTicketThroughByNullChk(PaTicketThrough entity) {
		SQL sql = new SQL().UPDATE("pa_ticket_through");
	 ...
	 * @return
	 */
	public String deletePaTicketThrough(int id) {
		SQL sql = new SQL().DELETE_FROM("pa_ticket_through");
		sql.WHERE("id = #{id}");
		return sql.toString();
	}
	/**
