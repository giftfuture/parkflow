 ...
 * provider
 */
public class PaTicketOrdinaryProvider {
	 ...
	 * @return
	 */
	public String insertPaTicketOrdinary(PaTicketOrdinary entity) {
		SQL sql = new SQL().INSERT_INTO("pa_ticket_ordinary");
	 ...
	 * @return
	 */
	public String selectPaTicketOrdinaryByCount(PaTicketOrdinary entity) {
		SQL sql = new SQL().SELECT("count(*)").FROM("pa_ticket_ordinary");
	 ...
	 * @return
	 */
	public String selectPaTicketOrdinaryByLimt(PaTicketOrdinary entity) {
		SQL sql = new SQL().SELECT("*").FROM("pa_ticket_ordinary");
					if(!Objects.isNull(entity.getId())) {sql.WHERE("id = #{id}");}
			if(!Strings.isNullOrEmpty(entity.getName())) {sql.WHERE("name = #{name}");}
			if(!Objects.isNull(entity.getPrice())) {sql.WHERE("price = #{price}");}
			if(!Strings.isNullOrEmpty(entity.getType())) {sql.WHERE("type = #{type}");}
	 ...
	 * @return
	 */
	public String selectPaTicketOrdinary(PaTicketOrdinary entity) {
		SQL sql = new SQL().SELECT("*").FROM("pa_ticket_ordinary");
			if(!Objects.isNull(entity.getId())) {sql.WHERE("id = #{id}");}
			if(!Strings.isNullOrEmpty(entity.getName())) {sql.WHERE("name = #{name}");}
			if(!Objects.isNull(entity.getPrice())) {sql.WHERE("price = #{price}");}
	 ...
	 * @return
	 */
	public String updatePaTicketOrdinary(PaTicketOrdinary entity) {
		SQL sql = new SQL().UPDATE("pa_ticket_ordinary");
				sql.SET("name = #{name}");
		sql.SET("price = #{price}");
		sql.SET("type = #{type}");
	 ...
	 * @return
	 */
	public String updatePaTicketOrdinaryByNullChk(PaTicketOrdinary entity) {
		SQL sql = new SQL().UPDATE("pa_ticket_ordinary");
	 ...
	 * @return
	 */
	public String deletePaTicketOrdinary(int id) {
		SQL sql = new SQL().DELETE_FROM("pa_ticket_ordinary");
		sql.WHERE("id = #{id}");
		return sql.toString();
	}
	/**
