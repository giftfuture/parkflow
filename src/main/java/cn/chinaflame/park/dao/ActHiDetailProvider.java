 ...
 * provider
 */
public class ActHiDetailProvider {
	 ...
	 * @return
	 */
	public String insertActHiDetail(ActHiDetail entity) {
		SQL sql = new SQL().INSERT_INTO("act_hi_detail");
		sql.VALUES("TYPE_,PROC_INST_ID_,EXECUTION_ID_,TASK_ID_,ACT_INST_ID_,NAME_,VAR_TYPE_,REV_,TIME_,BYTEARRAY_ID_,DOUBLE_,LONG_,TEXT_,TEXT2_", "#{type},#{procInstId},#{executionId},#{taskId},#{actInstId},#{name},#{varType},#{rev},#{time},#{bytearrayId},#{double},#{long},#{text},#{text2}");
		return sql.toString();
	}
	/**
	 * 按条件查询总记录数
	 ...
	 * @return
	 */
	public String selectActHiDetailByCount(ActHiDetail entity) {
		SQL sql = new SQL().SELECT("count(*)").FROM("act_hi_detail");
			...
			if(!Strings.isNullOrEmpty(entity.getActInstId())) {sql.WHERE("actInstId = #{actInstId}");}
			...
			if(!Objects.isNull(entity.getDouble_())) {sql.WHERE("double_ = #{double_}");}
			if(!Objects.isNull(entity.getLong_())) {sql.WHERE("long_ = #{long_}");}
	 ...
	 * @return
	 */
	public String selectActHiDetailByLimt(ActHiDetail entity) {
		SQL sql = new SQL().SELECT("*").FROM("act_hi_detail");
					if(!Strings.isNullOrEmpty(entity.getId())) {sql.WHERE("id = #{id}");}
			if(!Strings.isNullOrEmpty(entity.getType())) {sql.WHERE("type = #{type}");}
			if(!Strings.isNullOrEmpty(entity.getProcInstId())) {sql.WHERE("procInstId = #{procInstId}");}
			if(!Strings.isNullOrEmpty(entity.getExecutionId())) {sql.WHERE("executionId = #{executionId}");}
	 ...
	 * @return
	 */
	public String selectActHiDetail(ActHiDetail entity) {
		SQL sql = new SQL().SELECT("*").FROM("act_hi_detail");
			if(!Strings.isNullOrEmpty(entity.getId())) {sql.WHERE("id = #{id}");}
			if(!Strings.isNullOrEmpty(entity.getType())) {sql.WHERE("type = #{type}");}
			if(!Strings.isNullOrEmpty(entity.getProcInstId())) {sql.WHERE("procInstId = #{procInstId}");}
	 ...
	 * @return
	 */
	public String updateActHiDetail(ActHiDetail entity) {
		SQL sql = new SQL().UPDATE("act_hi_detail");
				sql.SET("id = #{id}");
		sql.SET("type = #{type}");
		sql.SET("procInstId = #{procInstId}");
		sql.SET("executionId = #{executionId}");
		sql.SET("taskId = #{taskId}");
		sql.SET("actInstId = #{actInstId}");
		sql.SET("name = #{name}");
		sql.SET("varType = #{varType}");
		sql.SET("rev = #{rev}");
		sql.SET("time = #{time}");
		sql.SET("bytearrayId = #{bytearrayId}");
		sql.SET("double_ = #{double_}");
		sql.SET("long_ = #{long_}");
		sql.SET("text = #{text}");
		sql.SET("text2 = #{text2}");
	 ...
	 * @return
	 */
	public String updateActHiDetailByNullChk(ActHiDetail entity) {
		SQL sql = new SQL().UPDATE("act_hi_detail");
			...
			if(!Strings.isNullOrEmpty(entity.getActInstId())) {sql.SET("actInstId = #{actInstId}");}
	 ...
	 * @return
	 */
	public String deleteActHiDetail(int id) {
		SQL sql = new SQL().DELETE_FROM("act_hi_detail");
		sql.WHERE("ID_ = #{ID_}");
		return sql.toString();
	}
	/**
