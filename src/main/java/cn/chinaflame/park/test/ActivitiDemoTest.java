...
import cn.ParkflowApplication;
import cn.chinaflame.park.model.PaStaff;
import cn.chinaflame.park.service.impl.ActivityDemoServiceImpl;  
...
@SpringBootTest(classes=ParkflowApplication.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)   //ParkflowApplication.class
...
public class ActivitiDemoTest {
	 @Resource  
	    ...
	    public void setTasksVarEntity() {  
	        List<Task> lists = activityDemoServiceImpl.findTasksByUserId("sly1");  
	        for(Task task : lists) {  
	        	PaStaff p = new PaStaff();  
	        	p.setStaffname("翠花");
	            p.setId(20L);    
	            p.setStaffno("@@@@@@@@@@@@@@22222222");
	    ...
	    public void getTasksVarEntity() {  
	        ...
	        for(Task task : lists) {  
	            ...
	            PaStaff p = (PaStaff)taskService.getVariable(task.getId(), "人员信息(添加固定版本)");    
	            System.out.println(" 请假人：  "+p.getStaffname()+"  请假天数：  "+p.getId()+"   请假时间："+ p.getCreatetime()+ "   请假原因： "+p.getStaffno());    
	        }  
	    }  
